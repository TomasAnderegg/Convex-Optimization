...
X = cp.vstack([cp.hstack([E, H]), cp.hstack([H.T, F])])
con.append(cp.diag(X) >= 0)

# Dominance of diagonal elements constraints
for i in range(2*N):
    for j in range(i+1, 2*N):
        con.append(cp.norm(cp.hstack([2*X[i,j], X[i,i] - X[j,j]]), 2) <= X[i,i] + X[j,j])
