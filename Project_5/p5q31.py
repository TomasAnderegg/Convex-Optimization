...
# ------------------------------------
# Our utils function (used for plotting our results)
# ------------------------------------
...
def create_plots(problem, results, L):
    ...
    plt.savefig(f"Problem_{problem}_L_{L}.png", dpi=300)

def compare_algos(results, L_list):
    algos = ["D", "G", "FW"]
    ...
    for L in L_list:
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(f"Comparison_L_{L}.png", dpi=300)
...
FW (Algorithm 1)
"""
def problem_fw(A_all, d, k, n_repeats, L, epsilon=1e-3, T=1000):
    L_max, _, _ = A_all.shape # = (400, 5, 2) which are L, d, K
    results = {}
    pi_values = np.zeros((n_repeats, L))
    runtimes = []
...
if RUN_FW:
    ...
    for L in L_list:
        results["FW"][L] = problem_fw(A_all, d, K, n_repeats, L)
        create_plots("FW", results["FW"][L], L)

if (RUN_COMPARISON and RUN_D_OPT and RUN_G_OPT and RUN_FW): # results must have all lists in dict
